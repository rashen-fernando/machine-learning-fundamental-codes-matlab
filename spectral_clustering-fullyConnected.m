...
for i=1:size(Y,2)
    for j=i:size(Y,2)
        a = Y(:,i)-Y(:,j); 
        W(i,j)=exp(-(a'*a)/(2*sig*sig));
        W(j,i)=W(i,j);
    end
    P = [P sum(W(i,:))];
end
%imshow(W);hold on;title(['adjacency matrix of yale data set - fully connect method (sigma = ',num2str(sig),')/PCA is applied'])
Degree_matrix = diag(P);
Lap = Degree_matrix - W;
[l,d] = eigs(Lap,165);

S=diag(d);
bar(S);hold on;title(['eigen values for sigma = ',num2str(sig)]);
[i,g]=max(abs(diff(S)));


bar(l(:,g));hold on;title([num2str(g),'th eigan vector for sigma = ',num2str(sig)])
