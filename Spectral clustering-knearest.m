...
%imshow(W);hold on;title(['adjacency matrix of yale data set - k nearest graph (max nearest members = ',num2str(e),')/PCA is applied'])
Degree_matrix = diag(P);
Lap = Degree_matrix - W;
[l,d] = eigs(Lap,165);

S=diag(d);
bar(S);hold on;title(['eigen values for max no of neighbors = ',num2str(e)]);
[i,g]=max(abs(diff(S)));


bar(l(:,g));hold on;title([num2str(g),'th eigan vector for k neihbors = ',num2str(e)])
